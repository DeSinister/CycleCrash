...
class FetchData():
    ...
    def prepare_data(self):
        for i in range(self.length_videos):
            if self.df['Right of Way'][i] != -1 and self.df['Duration'][i]>=self.segment_duration:
                framings = self.create_vid_segments(f"{dataset_dir}/{int(self.df['File Name'][i])}_{int(self.df['Counter'][i])}.mp4", self.df['Duration'][i])
                self.frames_list.append(framings)
                self.labels.append(torch.stack([torch.tensor(self.df['Right of Way'][i], dtype=torch.float32) for x in range(len(framings))]))
...
train_dataset = FetchData(csv_path = csv_path, video_dir=dataset_dir, length = 70, segment_duration= segment_length)
...
val_dataset = FetchData(csv_path = val_csv_path, video_dir=dataset_dir, length = 30, segment_duration= segment_length)
