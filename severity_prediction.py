    ...
    def prepare_data(self):
        for i in range(self.length_videos):
            if  self.df['Duration'][i]>=self.segment_duration:
                ...
                self.labels.append(torch.stack([torch.tensor(self.df['Severity Score'][i], dtype=torch.float32) for x in range(len(framings))]))
...
if __name__ == "__main__":
    ...
    metric2 = MulticlassF1Score(num_classes=5, average='macro').to(device)
