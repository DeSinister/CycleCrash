...
from .spatial_models.convnext import convnext_base, convnext_tiny
import yaml
...
class Model(nn.Module):
    ...
    def __init__(self, segment_length = 30):
        ...
        self.spatial_model = convnext_tiny(pretrained=True)
        ...
        spatial_output_dim = config.get('output_spatial_dim', {}).get('convnext_tiny')
    ...
    def forward(self, x, enc_self_mask=None, dec_self_mask=None, dec_enc_mask=None):
        ...
        # print("Input: ",x_shape, " to ", x.shape)
        # x = torch.stack([self.spatial_model(frame) for frame in x])
